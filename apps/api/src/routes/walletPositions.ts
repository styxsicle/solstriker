...
export function registerWalletPositionRoutes(app:FastifyInstance, prisma:PrismaClient, nodeEnv:string) {
  ...
  app.get('/api/wallet-positions',async(request,reply)=>{
    ...
    const latest=await latestCompletedRunByWallet(prisma); const runIds=[...latest.entries()].filter(([id])=>!q.data.walletId||id===q.data.walletId).map(([,runId])=>runId);
    const {page,pageSize,walletId,tokenId,status,confidence}=q.data; const where={reconstructionRunId:{in:runIds},...(walletId?{trackedWalletId:walletId}:{}),...(tokenId?{tokenId}:{}),...(status?{status}:{}),...(confidence?{confidence}:{})};
  ...
  app.get('/api/wallet-positions/:walletId',async(request,reply)=>{const {walletId}=request.params as any;const q=querySchema.safeParse(request.query);if(!q.success)return reply.code(400).send({error:'validation_error'});const wallet=await prisma.trackedWallet.findUnique({where:{id:walletId}});if(!wallet)return reply.code(404).send({error:'wallet_not_found'});const latest=await latestCompletedRunByWallet(prisma);const runId=latest.get(walletId);const where={trackedWalletId:walletId,reconstructionRunId:runId??'__none__',...(q.data.status?{status:q.data.status}:{}),...(q.data.confidence?{confidence:q.data.confidence}:{}),...(q.data.tokenId?{tokenId:q.data.tokenId}:{})};const [items,total]=await Promise.all([prisma.walletPosition.findMany({where,include:{token:true,matches:true},orderBy:[{openedAt:'desc'},{id:'asc'}],skip:(q.data.page-1)*q.data.pageSize,take:q.data.pageSize}),prisma.walletPosition.count({where})]);return{wallet:{id:wallet.id,address:wallet.address,label:wallet.label,emoji:wallet.emoji},items:items.map(positionDto),page:q.data.page,pageSize:q.data.pageSize,total};});
  app.get('/api/wallet-profiles',async(request)=>{const q=querySchema.parse(request.query);const latest=await latestCompletedRunByWallet(prisma);const pairs=[...latest.entries()].filter(([walletId])=>!q.walletId||walletId===q.walletId);const selected=pairs.slice((q.page-1)*q.pageSize,q.page*q.pageSize);const profiles=await prisma.walletBehaviorProfile.findMany({where:{OR:selected.map(([trackedWalletId,reconstructionRunId])=>({trackedWalletId,reconstructionRunId}))},include:{trackedWallet:{select:{address:true,label:true,emoji:true}},reconstructionRun:{select:{method:true,status:true,completedAt:true}}}});profiles.sort((a,b)=>a.trackedWalletId.localeCompare(b.trackedWalletId));return{items:await Promise.all(profiles.map(async p=>({...profileDto(p),knownBuySizesSol:await knownBuySizes(prisma,p.trackedWalletId)}))),page:q.page,pageSize:q.pageSize,total:pairs.length};});
  app.get('/api/wallet-profiles/:walletId',async(request,reply)=>{const {walletId}=request.params as any;const latest=await latestCompletedRunByWallet(prisma);const runId=latest.get(walletId);const p=runId?await prisma.walletBehaviorProfile.findFirst({where:{trackedWalletId:walletId,reconstructionRunId:runId},include:{trackedWallet:true,reconstructionRun:true},orderBy:[{calculatedAt:'desc'},{id:'desc'}]}):null;if(!p)return reply.code(404).send({error:'profile_not_found'});return{...profileDto(p),knownBuySizesSol:await knownBuySizes(prisma,walletId)};});
